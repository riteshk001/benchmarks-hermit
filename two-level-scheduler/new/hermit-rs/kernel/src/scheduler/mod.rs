...
pub mod fastrand;
use crate::scheduler::fastrand::XorShiftRng;
...
pub(crate) struct PerCoreScheduler {
	...
	/// Random number generator for this core
    rng: XorShiftRng,
...
impl PerCoreScheduler {
	...
	pub unsafe fn spawn(
		...
		//Stock the task in the global task list
		WAITING_TASKS.lock().insert(tid, VecDeque::with_capacity(1));
   		TASKS.lock().insert(tid, TaskHandle::new(tid, prio, core_id));
	...
	pub fn run() -> ! {
		...
		loop {
			...
			// do housekeeping
			#[cfg(feature = "smp")]
			core_scheduler.check_input(); // suppr later 
			// ask for task when ready queue is empty
			if core_scheduler.ready_queue.is_empty() {
				core_scheduler.cleanup_tasks();
            	global_queue::refill_from_global_queue(core_scheduler);
			}
...
pub(crate) fn add_current_core() {
	...
	let boxed_scheduler = Box::new(PerCoreScheduler {
		...
		rng : XorShiftRng::new_for_core(core_id as usize),
	});
